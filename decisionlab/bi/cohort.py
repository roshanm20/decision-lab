"""Monthly cohort retention table from a transactions CSV.

Moved into the package from tools/cohort_table.py on 23 September 2026,
logic unchanged. The old path still works.

The CSV needs customer_id and order_date, and optionally revenue. Rename
the columns with the flags if yours differ. Dates are read as YYYY-MM-DD,
and anything longer is cut to the first ten characters, so timestamps like
2025-04-17T09:31:00Z work.

The one design decision that matters: months a cohort has not reached yet
are left blank instead of printed as zero retention, and the average row
only uses cohorts that have reached each month. Most cohort tables get this
wrong and it makes recent cohorts look like they collapsed. Standard
library only.
"""
...
TOOL = {
    ...
    "summary": "Monthly cohort retention from a transactions CSV that leaves unreached months blank instead of showing them as zero.",
    "doc": "docs/tools/cohort.md",
}




def month_key(value: str, latest: date = None) -> str:
    """Return YYYY-MM from a date string. Raises ValueError on anything that is
    not a real calendar date, and on dates after `latest` (default: today),
    because one mistyped year like 2205 would otherwise switch off the blanking
    of unreached months for the whole table."""
    ...
    try:
        ...
        raise ValueError(f"cannot read a real date from {value!r}") from None
    if parsed > (latest or date.today()):
        raise ValueError(f"date {value!r} is in the future, probably a typo")
...
def make_demo(path=None, seed=7):
    """Synthetic data for testing. Retention decays, later cohorts are worse."""
    if path is None:
        path = os.path.join(tempfile.mkdtemp(prefix="decisionlab-"), "demo_transactions.csv")
    ...
    with open(path, "w", newline="", encoding="utf-8") as fh:
        ...
        for cohort_index in range(12):
            ...
            for _ in range(rng.randint(90, 140)):
                cid += 1
                # Later cohorts retain a little worse. This is a choice made for the
                # demo so the blank cells are easy to see, not a real-world claim.
...
def add_arguments(parser: argparse.ArgumentParser) -> None:
    parser.add_argument("csv_path", nargs="?", help="path to the transactions CSV")
    parser.add_argument("--id-col", default="customer_id")
    parser.add_argument("--date-col", default="order_date")
    ...
    parser.add_argument("--demo", action="store_true",
                        help="generate synthetic demo data in a temp folder and run on it")
...
def run(args: argparse.Namespace) -> int:
    ...
    if args.demo:
        path = make_demo()
    if not path:
        raise SystemExit("give a CSV path, or use --demo")
